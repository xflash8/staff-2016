<?php
class Mutasi_Rekening_Utama_Card extends Card{
	var $rekening;
	var $number = '7772227714';
	var $monthName;
	var $max;
	var $start;
	var $limit = false;
	var $pagetitle = "Mutasi 7772227714";
	var $table = 'mutasi';
	...
	function get_table()
	{
		...
		$this->table_obj = new $c($this->number);
			...
			<script>
				$(function() {		
					...
					$('#resync').click(function(e){
						...
						t = $(this);
						$.ajax({
							url: '?ajax=resync_mutasi',
							data: {mth: get.mth, yr:get.yr},
							type: 'POST',
							beforeSend: function(){
								console.log('mulai ...');
								t.val('Syncing ...');
							...
							success: function(r){
								if(r.new_credit){
									alert('ditemukan '+ r.new_credit.length + 'mutasi baru. Harap Refresh halaman');
								}else{
									console.log('gak ada transaksi baru');
									console.log(r);
								}
							}, 
							error: function(e){
					...
					$('.ajax-action').click(function(ev){
						ev.preventDefault();
						data = {user_id: $(this).attr('data-id'), mutasi_id: $(this).attr('data-mutasi_id')}
